...
class Trainer:
    ...
    def save_checkpoint(self, epoch, is_best=False, filename=None):
        ...
        self.path_join = os.path.join(self.args.ckpt_dir, filename)
        print('有执行保存',self.path_join)
        torch.save(state, self.path_join)
        if is_best:
            shutil.copy(self.path_join, os.path.join(self.args.ckpt_dir, 'checkpoint_best.pth.tar'))
